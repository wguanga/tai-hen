...
├── .claude/                   ← Agent 工作区（gitignore，本地）
│   ├── AGREEMENTS.md          ← 协作约定（测试、提交、交互）
│   └── testing.md             ← 测试规约详细版
...
完整 API 文档见 `docs/api-reference.md`，以下是主要端点概览：
...
新字段通过 `db.MIGRATIONS` 幂等迁移。详见 `docs/db-schema.md`。
...
- Windows 关闭 Electron 时 Python 子进程偶尔残留（见 `docs/dev-tips.md#49`）— 代码已加 `taskkill /T /F` 但极端情况下仍可能遗漏
